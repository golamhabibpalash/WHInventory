const App = {
    setup() {
        ...
        const services = {
            getMainData: async () => {
                try {
                    const response = await AxiosManager.get('/Brand/GetBrandList', {});
                    return response;
                } catch (error) {
                    throw error;
                }
            ...
            createMainData: async (name, description, createdById) => {
                try {
                    const response = await AxiosManager.post('/Brand/CreateBrand', {
            ...
            updateMainData: async (id, name, description, updatedById) => {
                try {
                    const response = await AxiosManager.post('/Brand/UpdateBrand', {
            ...
            deleteMainData: async (id, deletedById) => {
                try {
                    const response = await AxiosManager.post('/Brand/DeleteBrand', {
                        id, deletedById
                    });
                    return response;
                } catch (error) {
        ...
        const handler = {
            handleToggleStatus: async function (id) {
                try {
                    const response = await services.toggleStatus(id, StorageManager.getUserId());
        ...
        Vue.onMounted(async () => {
            try {
                await SecurityManager.authorizePage(['Brands']);
                ...
                mainGrid.obj = new ej.grids.Grid({
                    ...
                    rowSelected: () => {
                        if (mainGrid.obj.getSelectedRecords().length == 1) {
                            mainGrid.obj.toolbarModule.enableItems(['EditCustom', 'DeleteCustom', 'ToggleStatusCustom'], true);
                        } else {
                            mainGrid.obj.toolbarModule.enableItems(['EditCustom', 'DeleteCustom', 'ToggleStatusCustom'], false);
                        }
                    },
                    ...
                    toolbarClick: async (args) => {
                        ...
                        if (args.item.id === 'ToggleStatusCustom') {
                            if (mainGrid.obj.getSelectedRecords().length) {
                                const selectedRecord = mainGrid.obj.getSelectedRecords()[0];
