using Application.Features.NumberSequenceManager.Commands;
...
public class NumberSequenceController : BaseApiController
{
    ...
    [Authorize]
    [HttpPost("UpdateNumberSequence")]
    public async Task<ActionResult<ApiSuccessResult<UpdateNumberSequenceResult>>> UpdateNumberSequenceAsync(UpdateNumberSequenceRequest request, CancellationToken cancellationToken)
    {
        var response = await _sender.Send(request, cancellationToken);

        return Ok(new ApiSuccessResult<UpdateNumberSequenceResult>
        {
            Code = StatusCodes.Status200OK,
            Message = $"Success executing {nameof(UpdateNumberSequenceAsync)}",
            Content = response
        });
    }
